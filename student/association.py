...
class Association:
    ...
    def get_closest_track_and_meas(self):
        ...
        # Sanity check in case association matrix is empty. i.e. full of inf
        if np.min(A) == np.inf:
            return np.nan, np.nan
        
        # get indices of minimum entry
        ij_min = np.unravel_index(np.argmin(A, axis=None), A.shape) # unravel converts flatten index to 2D-index
        ind_track = ij_min[0]
        ind_meas = ij_min[1]
    ...
    def gating(self, MHD, sensor): 
        ...
        # Degrees Of Freedom (DOF) for I-CDF is equivalente to features-1
